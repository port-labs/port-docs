...
We will create a self-service actions to handle changing the status and the assignee of an issue. 
...
You can update a Jira issue by leveraging on Port's **synced webhooks** and **secrets** to directly interact with the Jira's API.   
...
5. For assignee changes: Select a Jira user to assign the ticket to
6. Click on `Execute`
7. Done! wait for the ticket to be updated in Jira
