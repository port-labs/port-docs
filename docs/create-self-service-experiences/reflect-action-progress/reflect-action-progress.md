...
:::info Github backend
When using a `Github workflow` as the action backend, Port will automatically update the status of the action run to `SUCCESS` or `FAILURE` according to the result of the Github workflow, so no manual update is required.
:::
