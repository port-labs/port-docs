...
With the IP restriction feature, you can define a set of allowed IP addresses on your account. Any user attempting to log in with an IP address that does not match an address on the allowed list will receive an error and they will not have the ability to proceed.

The same applies to API requests made to Port's API (either by a user, script or self-hosted integration) - requests made from an IP address that does not match an address on the allowed list will receive an error and the request will fail.
