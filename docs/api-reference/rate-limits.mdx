...
A rate limit restricts the number of requests or actions allowed within a certain timeframe to prevent overuse or abuse.
...
Port API's rate limits are enforced at the **IP level**. The table below shows the rate limit assigned to each API:
...
### Use selectors in Ocean integrations

The Ocean port-app-config `selector` and the `query` keys let you filter exactly which objects from the specified kind will be ingested to the software catalog.
In order to make less requests to Port, try to use Selectors as much as you can, to filter-out non-relevant data, Making unnecessary requests.
For example, if you try to fetch all Pull requests from Gitlab, Make sure you are filtering out closed PRs if you don't have any use for those.
