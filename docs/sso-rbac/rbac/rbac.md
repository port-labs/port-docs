...
When creating users via integrations or GitOps workflows, the `status` field is optional and should typically be omitted. Port automatically manages user status based on authentication events for SSO users. Non-SSO users will be created with the `Disabled` status.
:::
