...
To keep things short and simple, let's assume you only have a production environment. We'll use Port's [Terraform provider](../api-methods/terraform.md) to create an Entity for it.
...
Below is a `config.yaml` configuration file to use as the [Kubernetes Exporter](../exporters/k8s-exporter/introduction.md) config:
