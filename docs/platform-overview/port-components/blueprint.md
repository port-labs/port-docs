...
| `agent` | `boolean` | Defines whether to use [Port Agent](../self-service-actions/port-execution-agent) for execution or not. <br></br> Can be added only if `type` is set to `WEBHOOK` | Either `true` or `false`    |
