...
There are two main methods to access the monitoring metrics and sync status:
...
1. **Sync Status Tab**  
    ...
    - **Sync by kind**: Shows the montioring metrics and sync status for each `kind`.  
      - **Sync stages overview**:
        ...
        The next step is mapping these objects according to the integration's [mapping configuration](https://docs.port.io/build-your-software-catalog/customize-integrations/configure-mapping).  
          ...
          - Failed: Number of objects that failed to map, you can find more information about the failure reasons in the **Troubleshooting tips** section below or refer to the `Event log` tab.
        - **Ingest to Port**:  
        The final step is ingesting the mapped objects to Port.  
          In this section, you will find the following metrics:
          - Ingested: Number of objects that were ingested successfully.
          - Not changed: Objects were already up to date, so no changes were made.
          - Failed: Number of objects that failed to ingest, to know more about the failure reasons, see the `Event log` tab.
      - **Details**: Displays the duration of the sync.
    - **Reconciliation**:  
    This section provides visibility into both the reingestion and deletion processeses.  
      When the [`createMissingRelatedEntities`](https://docs.port.io/build-your-software-catalog/sync-data-to-catalog/git/azure-devops/advanced#using-advanced-configurations) flag is set to `false`, Port will **not** automatically create related entities that are referenced but not yet present in Port.
