...
import LiveEventsNote from "./_live_events_note.mdx";

Using this installation option means that the integration will be hosted by Port, with a customizable resync interval to ingest data into Port.

<h3>Live event support</h3>

{props.showLiveEventsGuide && <LiveEventsNote />}
...
{props.showSelfHostedTip !== false && (
  <Admonition type="tip" title="Self-hosted installation">
    Alternatively, you can install the integration using the <Link to="?installation-methods=real-time-self-hosted#setup">Self-hosted</Link> method to update Port in real time using webhooks.
  </Admonition>
)}
...
5. {props.showUseCustomSettingsStep && (
     <>Toggle on <code>Use Custom Settings</code> and </>
   )}Configure the `integration settings` and `application settings` as you wish (see below for details).
