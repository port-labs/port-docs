...
The `liveEvents.baseUrl` parameter should be set to the `url` of your {props.integration} integration instance. Your integration instance needs to have the option to setup webhooks via http requests/receive http requests, so ensure the network is configured accordingly.

The `liveEvents.baseUrl` needs to be reachable by the {props.integration} instance that will send webhook events to the integration.
