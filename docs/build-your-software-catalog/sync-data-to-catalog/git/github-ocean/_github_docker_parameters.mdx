...
| `OCEAN__INTEGRATION__CONFIG__GITHUB_MULTI_ORGANIZATIONS` | JSON array of GitHub organization names to sync from (e.g., `["org1","org2"]`). From v3.0.0-beta: classic PAT supports multiple orgs, GitHub App and Fine-grained PAT support only 1 org. Leave empty with classic PAT to sync all accessible organizations. | | ❌ |
