...
#Ai! generate a sample values.yml template from these helm variables, add it to a <details> element like is done in the argocd example, replace this helm command to reference the a values file rather having all variables set in the cmdline. There should be two details elements depending on if the example is being shown when "app" or not
...
<Tabs groupId="liveEvents" queryString="live-events">
    <TabItem value="kubernetes" label="Kubernetes">
      ...
      1.  **Ingress Configuration**: Enable the creation of an Ingress resource and specify the hostname that will be used to access the integration from the internet.
      2.  **Base URL**: Provide the integration with its public URL. This is used to register the webhook in GitHub.
...
#Ai! generate a sample values.yml template from these helm variables, add it to a <details> element like is done in the argocd example, replace this helm command to reference the a values file rather having all variables set in the cmdline. There should be two details elements depending on if the example is being shown when "app" or not
      ...
      {`helm upgrade --install github-ocean port-labs/port-ocean \\
        ...
        --set integration.eventListener.type="POLLING" \\
      	...
      	--set ingress.path="/github-ocean" \\
      	--set integration.extraEnv[0].name="OCEAN__BASE_URL" \\
      	--set integration.extraEnv[0].value="<your-domain>/github-ocean"`}
          </CodeBlock>
        ) : (
          <CodeBlock language="bash" showLineNumbers>
      {`helm upgrade --install github-ocean port-labs/port-ocean \\
      ...
      When you set the `OCEAN__BASE_URL`, Ocean will attempt to automatically create the webhook in GitHub for you.
    </TabItem>

    <TabItem value="docker" label="Docker">
      
      When running the integration with Docker, you need a public URL to receive live events from GitHub. This can be achieved by using a reverse proxy to expose the container to the internet.

      <details>
        <summary><b>Local Testing with Ngrok (Click to expand)</b></summary>

        For local testing, you can use [Ngrok](https://ngrok.com/) to create a secure tunnel to your machine. This will give you a temporary public URL that forwards traffic to your Docker container.

        1.  Start a tunnel to port `8000`:
            ```bash
            ngrok http 8000
            ```
        2.  Ngrok will provide you with a public URL. It will look something like this:
            ```
            Forwarding                    https://c696-2693-1a59.ngrok-free.app -> http://localhost:8000
            ```
        3.  Copy the `https` URL and use it as the `OCEAN__BASE_URL` when running your Docker container:
            ```bash showLineNumbers
            docker run \
              -e OCEAN__BASE_URL="https://c696-2693-1a59.ngrok-free.app" \ # Replace with your ngrok URL
      ...
      ```nginx title="/etc/nginx/sites-available/github-ocean.conf" showLineNumbers
server {
    listen 80;
