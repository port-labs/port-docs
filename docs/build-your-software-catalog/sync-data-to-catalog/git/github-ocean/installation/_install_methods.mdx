...
<Tabs groupId="liveEvents" queryString="live-events">
    ...
    <TabItem value="docker" label="Docker">
      ...
      # AI! write a short guide on setting up a nginx reverse proxy server config to proxy requests to a hypothetical github-ocean.example-domain.com to our running docker container. This step should include the example config, the step to restart the nginx server, as well as the step to run our docker container using the proxied hostname as our base_url
      #### Setting up a with Nginx
