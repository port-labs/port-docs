...
spec:
  ...
  - host: <YOUR_DOMAIN> # e.g., github-ocean.yourcompany.com
    http:
      ...
      When you provide the `base_url` during the Helm installation, Ocean will attempt to automatically create the webhook in GitHub for you. If you are setting this up after installation, you can set this by running the install command again but with base_url set this time.
      ```sh showLineNumbers
        helm upgrade --install github-ocean port-labs/port-ocean \
        # ... be sure to set the other variables
        --set integration.version="1.0.5-beta"  
    ```
