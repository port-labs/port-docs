...
<Tabs groupId="liveEvents" queryString="live-events">
    <TabItem value="kubernetes" label="Kubernetes">
      ...
      A full example based on the installation command:
        {props.setupName == "app" ? (
        # AI! The command in the following CodeBlocks can't be copied and pasted due to zsh parsing issues
          <CodeBlock language="bash" showLineNumbers>
      {`helm upgrade --install github-ocean port-labs/port-ocean \\
        --set port.clientId="<PORT_CLIENT_ID>" \\
      	--set port.clientSecret="<PORT_CLIENT_SECRET>" \\
        ...
        --set integration.eventListener.type="POLLING" \\
      	...
      	--set sendRawDataExamples=true \\
      	--set scheduledResyncInterval=360 \\
      	--set ingress.enabled=true \\ # New
      	--set ingress.path="/github-ocean" \\ 
      	--set-json 'integration.extraEnv=[{"name":"OCEAN__BASE_URL", "value": "<your-domain>/github-ocean" }]'`}
          </CodeBlock>
        ) : (
          <CodeBlock language="bash" showLineNumbers>
      {`helm upgrade --install github-ocean port-labs/port-ocean \\
