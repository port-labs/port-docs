...
{`
  ...
  integration:
    ...
    eventListener:
      type: "POLLING"
    config:
      githubOrganizations: ["<GITHUB_ORGANIZATION>"]  # GitHub App supports only 1 org
...
`}
  ...
  integration:
    ...
    eventListener:
      type: "POLLING"
    config:
      githubOrganizations: ["<GITHUB_ORG_1>", "<GITHUB_ORG_2>"]  # From v.2.0.0-beta, classic PAT supports multiple orgs. Leave empty to sync all orgs
      githubHost: "<GITHUB_HOST>" # e.g https://api.github.com
    secrets:
...
:::note
Be sure to replace the `<GITHUB_TOKEN>` and organization placeholders with your actual values. If you are using a self-hosted GitHub instance, update the `githubHost` value to point to your instance.

For multi-organization support, use a **classic PAT token** and specify multiple organizations in the array.
:::
...
integration:
  ...
  config:
    ...
    githubOrganizations: ["<GITHUB_ORG_1>", "<GITHUB_ORG_2>"]  # From v.2.0.0, classic PAT supports multiple orgs
  secrets:
