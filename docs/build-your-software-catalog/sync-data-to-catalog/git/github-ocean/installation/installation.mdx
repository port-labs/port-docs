...
- Classic PAT: optional `githubMultiOrganizations: ["org1", "org2", "org3"]`. If omitted, all accessible orgs are synced.
- GitHub App and fine-grained PAT: use a single `githubOrganization`.
- Do not set both `githubOrganization` and `githubMultiOrganizations` at the same time.
