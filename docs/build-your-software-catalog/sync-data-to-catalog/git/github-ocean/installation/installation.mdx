...
Starting from **version 2.0.0-beta**, the GitHub integration supports syncing data from multiple GitHub organizations.
...
GitHub App authentication supports **exactly one organization** only. You must specify one organization in the `githubOrganizations` configuration parameter.
:::
