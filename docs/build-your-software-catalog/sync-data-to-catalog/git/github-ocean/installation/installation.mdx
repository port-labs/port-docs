...
- **Supported authentication method**: Multi-organization support is available only when you authenticate using a **classic PAT**. Syncing multi organizations is **not supported** when authenticating via **GitHub App** (including **GitHub app (via Port)** / self hosted setups)
