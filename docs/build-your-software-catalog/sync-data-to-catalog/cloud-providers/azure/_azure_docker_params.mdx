...
| `OCEAN__PROCESS_EXECUTION_MODE`      | Controls how kinds are executed. Allowed values: `multi_process` (each kind in a separate process), `single_process` (all kinds in one process). Recommended: `multi_process` for cloud provider integrations. | ❌       |
