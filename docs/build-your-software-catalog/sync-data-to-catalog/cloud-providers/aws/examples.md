...
Your [data model](https://app.getport.io/dev-portal/data-model) should now contain the `Service` blueprints:
...
2. Go to the `template-assets/aws/s3_bucket` folder from the [ingestion step](https://docs.getport.io/build-your-software-catalog/sync-data-to-catalog/cloud-providers/aws/Installation). We are going to add a relation from our S3 bucket configuration to the `Service` blueprint by editing two files:
