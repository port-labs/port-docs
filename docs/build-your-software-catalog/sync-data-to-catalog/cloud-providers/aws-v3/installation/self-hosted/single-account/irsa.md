...
We'll walk you through deploying the AWS integration using Helm with IRSA (IAM Roles for Service Accounts) on EKS to sync resources from a single AWS account.
...
**Solutions**:
- Verify the IAM role has `ReadOnlyAccess` policy attached.
