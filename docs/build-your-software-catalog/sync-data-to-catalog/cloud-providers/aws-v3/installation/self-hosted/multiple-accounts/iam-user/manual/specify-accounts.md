...
We'll walk you through deploying the AWS integration with IAM user authentication to sync resources from multiple AWS accounts by manually creating IAM roles in each member account and providing a manual list of role ARNs.
...
To use automatic account discovery with AWS Organizations, see [Automatic account discovery](/build-your-software-catalog/sync-data-to-catalog/cloud-providers/aws-v3/installation/self-hosted/multiple-accounts/iam-user/manual/automatic-account-discovery.md).
:::
