...
The following example adds another `image` blueprint, in addition to the `ciJob` blueprint shown in the previous example. In addition, it also adds a `image` relation. The Codefresh workflow template will create or update the relation between the 2 existing entities, allowing you to map the image to the ciJob that created it:

Here is the complete workflow `yml` to create both `image` entity, the `cijob` entity with a relation between them, and then print the complete
JSON of the new `image` entity:
...
metadata:
  ...
  annotations:
    port/version: "0.0.1"
    port/description: "This Workflow Template is used to create a image entity, a ciJob entity and then print the JSON of the new image"
spec:
  arguments:
    parameters:
  entrypoint: ci-tasks
