...
The following example adds a `comment` blueprint, in addition to the `issues` blueprint shown in the previous example. In addition, it also adds a `sentryIssuesEvent` relation. The webhook will create or update the relation between the 2 existing entities, allowing you to map which issue a comment is made on:
