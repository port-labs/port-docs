...
| `integration.config.hasMorePath` | Path to check if there are more pages. For Slack API, use `response_metadata.next_cursor` (the simple path works because empty strings are falsy, so pagination stops when the cursor is empty).                                                                                                                                                                         | response_metadata.next_cursor                                  | ✅        |
